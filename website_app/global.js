                ...
                if(data.status === 'succeeded') {
                    ...
                    if(data.sets.length != 0) {
                        $.each(data.sets, function(index, set) {
                            $('#setsList').append('<div class="setWrapper" onclick="window.location=\'?set=' + set.setId + '\'"><p>' + set.setName + '</p></div><button class="btn btn-danger deleteCardBtn" onclick="deleteSet(' + set.setId + ')">Delete</button>');
                        });
                    }else {
